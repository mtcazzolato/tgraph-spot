...
def main():
    ...
    Demo run
    """
    # mat_file = 'cross_associations/data/CP.mat'
    # mat_file = '/home/mirela/repository/bepe/wcw/spotting-results/adj_matrix.csv'
    # loaded_matrix = scipy.io.loadmat(mat_file)
    # matrix = Matrix(loaded_matrix['CP'])
    sparse_matrix = scipy.sparse.load_npz('/home/mirela/repository/bepe/wcw/spotting-results/sparse_matrix.npz')
    # print(sparse_matrix)
    matrix = Matrix(sparse_matrix)
    # print(loaded_matrix['CP'])
    cluster_search = ClusterSearch(matrix)
    cluster_search.run()
    scipy.sparse.save_npz('sparse_matrix.npz',
                          cluster_search._matrix._matrix)
    scipy.sparse.save_npz('transforomed_sparse_matrix.npz',
                          cluster_search._matrix.transformed_matrix)
    return 0
